...
const envSchema = z.object({
  ...
  AI_PROVIDER: z.enum(["gemini", "medgemma"]).default("gemini"),
  GEMINI_API_KEY: z.string().optional(),
  GEMINI_MODEL: z.string().default("gemini-2.0-flash"),
  MEDGEMMA_ENDPOINT: z.string().url().default("http://localhost:8000/v1/chat/completions"),
  MEDGEMMA_MODEL: z.string().default("medgemma-1.5-4b"),
  MEDGEMMA_TIMEOUT_MS: z.coerce.number().int().positive().default(30000),
  MEDGEMMA_RETRY_COUNT: z.coerce.number().int().min(0).default(3),
  MEDGEMMA_MAX_TOKENS: z.coerce.number().int().positive().default(2048),
  MEDGEMMA_TEMPERATURE: z.coerce.number().min(0).max(2).default(0.1),
